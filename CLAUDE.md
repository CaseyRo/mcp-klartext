...
2. `data/brands/*.md` are loaded into `BrandContext` entries. Active brands (May 2026 collapse): `casey` and `yorizon`. The casey brand additionally exposes a `## Registers` section (`personal`, `professional`) parsed by `_extract_registers()` in `voice.py` and surfaced as `BrandContext.registers`. Removed brands (`casey-berlin`, `cdit-works`, `storykeep`, `nah`) are intercepted by `_REMOVED_BRANDS` in `server.py` and return a migration error pointing at the correct `casey` + register combination.
...
**Tools exposed**:
- `generate_text_context(context, platform, language, register?, references?)` — one-shot that merges voice + brand + platform. For `context="casey"`, the optional `register` arg (`personal` or `professional`) merges the matching register overlay into `brand_context.register_overlay`. When the casey brand is requested without a register, the response signals `register_required: true` so callers know to disambiguate.
- `get_voice_dna`, `get_brand_context(context, register?)`, `get_platform_template`, `list_platforms` — granular accessors (read-only content). `get_brand_context` also accepts `register` for the casey brand.
- `scan_draft(text, brand?)` — runs the AI Bleed Scan rubric (see `data/skill.md`) programmatically via `mcp_klartext/scanner.py`. When `brand="casey"`, the May 2026 hard rules apply: `NO_EM_DASH` (high severity, zero tolerance), `NO_ALL_CAPS` (high severity, with a tech-acronym whitelist), and `ANTI_ANCHOR_PROXIMITY` (medium severity for Musk/Altman/supercharge/10x patterns). Pattern lists live in `LEXICAL_TELLS`, `PHRASE_TELLS`, `_ANTI_ANCHORS`, and `_ANTI_ANCHOR_PHRASE_TELLS` at the top of `scanner.py` — update those together with the rubric text in `skill.md` so instructions and enforcement stay in sync.
...
**Deployment**: Komodo stack `git-mcp-klartext-nebula` (see `komodo.toml`) deploys from the `main` branch on `nebula-1`, container `git-mcp-klartext-nebula-mcp-klartext-1` pulling `ghcr.io/caseyro/mcp-klartext:latest`, exposed at `https://mcp-klartext.cdit-dev.de/mcp` through the Cloudflare MCP Portal. The stack injects `MCP_API_KEY` (sourced from the Komodo variable `MCP_KLARTEXT_API_KEY`) — the env name must match what `server.py` reads or auth silently no-ops. Clients authenticate with portal-brokered tokens via OAuth, not the raw `MCP_API_KEY` (that's the upstream credential the portal uses).
